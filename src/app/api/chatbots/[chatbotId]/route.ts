...
export async function GET(req: NextRequest, { params }: { params: { chatbotId: string } }) {
  try {
    ...
    const chatbot = await Chatbot.findOne({ _id: params.chatbotId, userId });
    ...
    console.error(`Failed to fetch chatbot ${params.chatbotId}:`, error);
...
export async function PUT(req: NextRequest, { params }: { params: { chatbotId: string } }) {
    try {
        ...
        console.error(`Failed to update chatbot ${params.chatbotId}:`, error);
        return NextResponse.json({ message: "An error occurred." }, { status: 500 });
    }
}